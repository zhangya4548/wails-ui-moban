...
export default function RegisterPage() {
  ...
  async function register() {
    ...
    // todo 执行登录操作
    // 登录成功
    toast.success("👋 注册成功跳转登录页～");
    navigate("/login"); // 跳转到首页
  }
    ...
    <main style="--wails-draggable:drag" class="w-full h-screen">
      <div class="pt-52">
        <div class="mx-auto w-96 space-y-6 bg-white p-12 rounded-lg">
          <h1 class="text-center text-2xl font-semibold">注册</h1>
          ...
          <button type="submit" class={btnClass() + ' w-full'} onclick={register}>注册</button>
          <Show when={errMsg()}>
